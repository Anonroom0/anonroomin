...
import { hapticSelect, hapticSend } from '../../lib/haptics';
...
export default function SwipeableMessage({ children, onSwipe, disabled }) {
  ...
  const handleTouchMove = (e) => {
    ...
    if (diff < 0 && diff > -70) {
      // Firm tick right as the swipe crosses into "will trigger a reply"
      // territory, so releasing doesn't come as a surprise.
      if (diff <= -40 && translateX > -40) hapticSelect();
      setTranslateX(diff);
    }
  };
  ...
  const handleTouchEnd = () => {
    if (disabled) return;
    if (translateX <= -40) { hapticSend(); onSwipe(); }
    setTranslateX(0);
