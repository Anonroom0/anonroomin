 ...
 * the real username of an admin — the admin's actual display picture is
 * still shown (LiquidAvatar rings it gold + badges it) so admin stays
 * recognizable without exposing the underlying username.
 */
        ...
        <div style={{ display: 'flex', flexDirection: 'column', padding: '0 8px' }}>
          ...
          {results.map((user, index) => {
            ...
            return (
              <button
                key={user.id}
                onClick={() => { hapticTap(); playTap(); onSelectUser(user.id); }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    <span 
                      style={{ 
                        ...
                        color: identity.is_admin ? 'var(--admin-1)' : 'var(--paper)',
                    ...
                    {identity.is_admin && (
                      <span style={{ color: 'var(--admin-1)', display: 'flex', alignItems: 'center' }}>
                        {Vectors.AdminShield}
                      </span>
                    )}
                  </div>
