...
export function AudioBubble({ src, isOwn }) {
  ...
  const bubbleFill = isOwn ? 'var(--bubble-own)' : 'var(--bubble-other)';
  const playButtonBg = isOwn ? 'var(--ember)' : 'var(--glass-border)';
  const playButtonColor = isOwn ? 'var(--bubble-own-text)' : 'var(--bubble-other-text)';
  const activeBarColor = isOwn ? 'var(--bubble-own-text)' : 'var(--paper)';
