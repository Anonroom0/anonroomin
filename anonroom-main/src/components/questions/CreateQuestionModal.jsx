...
import { hapticSend, hapticTap } from '../../lib/haptics';
...
function CreateQuestionModalContent({ onCreated }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: '4px 20px 28px' }}>
      <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--paper)' }}>Create a Question</div>
      <div style={{ fontSize: 13.5, color: 'var(--dim)', marginTop: -12, lineHeight: 1.4 }}>
        Publish a prompt to your profile so anyone with the link can respond — anonymously if they choose.
      </div>
        ...
        {QUESTION_TYPES.map(({ value, label }) => {
          ...
          return (
            <button
              key={value}
              type="button"
              onClick={() => { hapticTap(); setQuestionType(value); }}
              style={{
                flex: 1,
                padding: '14px 0',
                borderRadius: 20,
        ...
        <textarea
          ...
          placeholder={
            questionType === 'personal'
              ? 'Write a personal question for your followers to answer honestly…'
              : questionType === 'general'
                ? 'Write an open question anyone can weigh in on…'
                : 'Select a question type above to begin writing…'
          }
