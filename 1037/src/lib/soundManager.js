 ...
 *
 * v2: every envelope's peak gain and duration were raised from v1 (which
 * kept several sounds, notably playTabSwitch, so quiet they were nearly
 * inaudible on a phone speaker at normal volume) so the whole app's sound
 * design reads as clearly felt rather than a faint background tick. Still
 * short, dry, and mechanical — no reverb/swoosh was added, just more
 * presence in the same character. Two new general-purpose sounds
 * (playTap, playOpen/playClose) were added for wiring sound into
 * interactions — buttons, sheet open/close — that had none before.
 * ========================================================================= */
...
/** Dry, low tactile click (~90ms) — filtered noise burst through a lowpass + fast gain envelope. */
export function playSend() {
  if (isMuted()) return;
  const ctx = getAudioContext();
  if (!ctx) return;
  scheduleNoiseClick(ctx, ctx.currentTime, { filterFreq: 1400, peakGain: 0.85, duration: 0.09 });
}

/** Higher, punchy single tick (~110ms) — short triangle blip. */
export function playReceive() {
  if (isMuted()) return;
  const ctx = getAudioContext();
  if (!ctx) return;
  scheduleTone(ctx, ctx.currentTime, { type: 'triangle', frequency: 880, peakGain: 0.65, duration: 0.11 });
}

/** Short, felt low thud (~60ms). */
export function playTabSwitch() {
  if (isMuted()) return;
  const ctx = getAudioContext();
  if (!ctx) return;
  scheduleTone(ctx, ctx.currentTime, { type: 'sine', frequency: 130, peakGain: 0.32, duration: 0.06 });
}

/** Two-tone rise — two quick dry clicks in succession (~130ms total), not a chime. */
export function playRefreshComplete() {
  if (isMuted()) return;
  const ctx = getAudioContext();
  if (!ctx) return;
  const now = ctx.currentTime;
  scheduleNoiseClick(ctx, now, { filterFreq: 750, peakGain: 0.7, duration: 0.06 });
  scheduleNoiseClick(ctx, now + 0.06, { filterFreq: 1500, peakGain: 0.7, duration: 0.07 }); // higher-filtered second click = the "rise"
}

/** Flat low buzz, no pitch bend (~130ms) — noticeably more present than a UI blip. */
export function playError() {
  if (isMuted()) return;
  const ctx = getAudioContext();
  if (!ctx) return;
  scheduleTone(ctx, ctx.currentTime, { type: 'square', frequency: 160, peakGain: 0.55, duration: 0.13, sustainUntil: 0.09 });
}

/** Quick bright click (~45ms) — generic button/toggle/swatch taps; lighter and higher than playSend so rapid browsing (color grids, style galleries) doesn't feel heavy. */
export function playTap() {
  if (isMuted()) return;
  const ctx = getAudioContext();
  if (!ctx) return;
  scheduleNoiseClick(ctx, ctx.currentTime, { filterFreq: 2000, peakGain: 0.55, duration: 0.045 });
}

/** Soft rising chirp (~90ms) — a sheet/modal opening. */
export function playOpen() {
  if (isMuted()) return;
  const ctx = getAudioContext();
  if (!ctx) return;
  scheduleTone(ctx, ctx.currentTime, { type: 'triangle', frequency: 540, peakGain: 0.5, duration: 0.09 });
}

/** Soft falling thud (~70ms) — a sheet/modal closing. */
export function playClose() {
  if (isMuted()) return;
  const ctx = getAudioContext();
  if (!ctx) return;
  scheduleTone(ctx, ctx.currentTime, { type: 'sine', frequency: 210, peakGain: 0.4, duration: 0.07 });
}
