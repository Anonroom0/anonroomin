...
function getTokens() {
  return {
    ink: getToken('--ink', '#070B14'),
    ink2: getToken('--ink-2', '#121A2E'),
    paper: getToken('--paper', '#F2F6FF'),
    dim: getToken('--dim', '#8B96B3'),
    ember: getToken('--ember', '#3D8BFF'),
  };
}
...
const CONFETTI_PALETTE = ['#3D8BFF', '#8B5CF6', '#2DD4A7', '#F5C64B', '#FB7EC0', '#3B82F6'];
...
function drawBackground(ctx, backgroundPreset) {
  ...
  if (type === 'dots') {
    ...
    ctx.clip();
    ctx.fillStyle = accent || 'rgba(120,170,255,0.16)';
    ...
    ctx.rect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
    ctx.clip();
    ctx.strokeStyle = dotColor || 'rgba(63,120,255,0.07)';
    ctx.lineWidth = 2;
    const spacing = 50;
    const diagonal = Math.sqrt(CANVAS_WIDTH ** 2 + CANVAS_HEIGHT ** 2);
    [45, -45].forEach((angle) => {
    ...
    ctx.strokeStyle = dotColor || 'rgba(120,170,255,0.16)';
    ctx.lineWidth = 3;
    ...
    ctx.fillStyle = dotColor || 'rgba(120,170,255,0.16)';
    const spacing = 40;
...
function readableTextColor(hex) {
  return relativeLuminance(hex) > 0.42 ? '#070B14' : '#FFFFFF';
}
...
function buildThemeRuntime(backgroundId, colorId) {
  ...
  const base = mixHex(accent, '#070B14', 0.42);
  const baseDeep = mixHex(accent, '#070B14', 0.62);
...
function resolveFill(fillKind, tokens) {
  ...
  if (fillKind === 'glass') return 'rgba(63,120,255,0.07)';
...
function applyBorder(ctx, borderKind, tokens) {
  ...
  if (borderKind === 'glass') { ctx.lineWidth = 2; ctx.strokeStyle = 'rgba(120,170,255,0.16)'; return true; }
...
function drawBodyCard(ctx, { x, y, width, height, bodyPreset, tokens, headerPreset }) {
  ...
  if (bodyPreset.confetti) {
    ...
    const palette = [headerPreset.pillBg, 'rgba(255,255,255,0.3)', 'rgba(90,150,255,0.20)'];
...
const NORMAL_TAG_DEFAULTS = {
  ...
  message: { label: 'Message', from: '#3D8BFF', to: '#5B8FFF' },
};
