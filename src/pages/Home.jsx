    ...
    try {
      ...
      if (userId) {
        ...
        finalThreads = await Promise.all((threadRows || []).map(async (t) => {
          ...
          return { ...t, otherUser: profilesById[otherId] || { id: otherId, username: 'Unknown User' }, unread_mention: count && count > 0 };
        }));
          ...
          >
            ...
            <div style={{ padding: '8px 16px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'relative', zIndex: 40 }}>
              <div style={{ display: 'flex', background: '#1C1D24', borderRadius: 20, padding: 4, boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.06)' }}>
                <button 
                  className="touch-bounce" onClick={() => setActiveTab('chats')} 
                ...
                >Chats</button>
                <button 
                  className="touch-bounce" onClick={() => setActiveTab('ask_me')} 
            ...
            <div style={{ position: 'absolute', top: 150, left: 0, right: 0, height: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 5, pointerEvents: 'none', transform: `translateY(${Math.min(pullDistance - 60, 0)}px)`, opacity: pullDistance > 10 ? 1 : 0, transition: isRefreshing ? 'transform 0.3s cubic-bezier(0.2, 0.8, 0.2, 1)' : 'none', color: 'var(--ember)' }}>
                                  ...
                                  <button key={thread.id} className={`chat-row stagger-item ${isActive ? 'active-chat' : ''}`} style={{ animationDelay: `${(groups.length + index) * 0.04}s` }} onClick={() => handleOpenChat(otherId, 'dm', thread.otherUser?.username)}>
                                    ...
                                    <div className="chat-row-content">
                                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                                        <span style={{ fontWeight: 600, fontSize: 16, display: 'flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{identity.name}{identity.is_admin && <span style={{ color: '#FFD700' }}>{Icons.AdminShield}</span>}</span>
