 ...
 * - Attachment preview bar (image/video/file thumbnail + caption + send) is
 *   now actually mounted — it existed as state/handlers before but never
 *   rendered, which is why captions had nowhere to go.
 * - Attachment sheet: Files / Image / Camera / Confession / Instagram.
 * - Confessions: anonymous-by-default posts, gradient bubble, pin button that
 *   cycles through them newest-first.
 ...
 * - Anonymous Mode, Admin Deletion, Pull-to-Refresh, Skeleton Loading kept
...
const CONFESSION_GRADIENT = 'linear-gradient(135deg, #7b2ff7, #f107a3)';
...
const Vectors = {
  ...
  Pin: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      <line x1="12" y1="17" x2="12" y2="22" /><path d="M5 17h14l-1.5-9.5a2 2 0 0 0-2-1.5H8.5a2 2 0 0 0-2 1.5z" />
    </svg>
  ),
};
...
function GroupLiquidAvatar({ url, name, size = 42, isAdmin = false, isAnon = false }) {
  const containerStyle = {
    ...
    overflow: 'hidden', boxShadow: 'inset 0 0 0 1px var(--glass-border)', userSelect: 'none'
  };

  if (isAnon) {
    return (
      <div style={{ ...containerStyle, background: 'var(--glass-border)', color: 'var(--dim)' }}>
        <div style={{ transform: 'scale(0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{Vectors.GhostSolid}</div>
      </div>
    );
  }
  if (isAdmin) {
    return (
      <div style={{ ...containerStyle, background: 'linear-gradient(135deg, #FFD700 0%, #FFA500 100%)', color: '#fff', fontSize: size * 0.35, fontWeight: 800 }}>ADM</div>
    );
  }
  if (url) {
    return (
      <div style={containerStyle}><img src={url} alt={name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /></div>
    );
  }
  const colors = [
  ...
  return (
    <div style={{ ...containerStyle, background: colors[colorIndex], color: '#ffffff', fontWeight: 700, fontSize: size * 0.4 }}>{getInitials(name)}</div>
  );
}
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 50, display: 'flex', alignItems: 'flex-end' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', background: 'var(--glass-strong)', backdropFilter: 'blur(30px)', borderRadius: '20px 20px 0 0', padding: 20 }}>
        <h3 style={{ margin: '0 0 12px', color: 'var(--ink)' }}>New Confession</h3>
        <textarea
          value={text} onChange={(e) => setText(e.target.value)} rows={4} placeholder="Type your confession…"
          style={{ width: '100%', border: '1px solid var(--glass-border)', borderRadius: 12, padding: 12, fontSize: 14, resize: 'none', boxSizing: 'border-box', background: 'var(--glass)', color: 'var(--ink)' }}
        />
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '12px 0', fontSize: 14, color: 'var(--ink)' }}>
          <input type="checkbox" checked={anon} onChange={(e) => setAnon(e.target.checked)} /> Post anonymously
        </label>
        <button
          onClick={() => { if (text.trim()) { onSubmit(text.trim(), anon); setText(''); } }}
          style={{ width: '100%', padding: 14, borderRadius: 16, border: 'none', background: 'var(--blue)', color: '#fff', fontWeight: 700, cursor: 'pointer' }}
        >
          Post Confession
        </button>
      </div>
...
function SwipeableMessage({ children, onSwipe, disabled }) {
  ...
  const handleTouchStart = (e) => { if (!disabled) touchStartX.current = e.touches[0].clientX; };
  const handleTouchMove = (e) => {
  ...
  return (
    <div
      ...
      style={{ transform: `translateX(${translateX}px)`, transition: translateX === 0 ? 'transform 0.25s cubic-bezier(0.2, 0.8, 0.2, 1)' : 'none', width: '100%', position: 'relative', touchAction: 'pan-y', willChange: 'transform' }}
    >
...
function SendButton({ canSend, sending, cooldownPercent }) {
  ...
  return (
    <button
      type="submit" disabled={!canSend || sending || isCoolingDown}
      style={{ position: 'relative', width: ringSize, height: ringSize, borderRadius: '50%', border: 'none', flexShrink: 0, background: isCoolingDown ? 'var(--glass)' : (canSend ? 'var(--blue)' : 'var(--glass-border)'), color: canSend ? '#fff' : 'var(--dim)', cursor: canSend && !isCoolingDown ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s', userSelect: 'none' }}
    >
      {isCoolingDown ? (
        <>
...
export default function GroupChat({ groupSlug, onBack, onGroupResolved }) {
  ...
  const renderMessageTextWithMentions = (messageText, isOwn) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('@') && part.length > 1) {
        ...
        return (
          <button key={i} onClick={() => handleMentionClick(username)} style={{ color: isOwn ? '#cce4ff' : 'var(--blue)', textDecoration: 'underline', background: 'none', border: 'none', padding: 0, fontWeight: 700, cursor: 'pointer', fontSize: 'inherit' }}>
            {part}
          </button>
        );
      }
  ...
  // CONFESSIONS
  // --------------------------------------------------------------------------
  async function handleConfessionSubmit(confessionText, anon) {
  ...
  if (groupStatus === 'error') {
    return (
      <div className="no-copy-text" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', flexDirection: 'column', gap: 16 }}>
        ...
        <button onClick={onBack} style={{ background: 'var(--blue)', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 12, cursor: 'pointer' }}>Go Back</button>
      </div>
    );
  }
    ...
    <div className="no-copy-text" style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative', height: '100%', overflow: 'hidden', zIndex: 1, userSelect: 'none', WebkitUserSelect: 'none' }}>
      ...
      {selectedMessages.length > 0 ? (
        ...
        <header style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 14, padding: '12px 20px', background: 'var(--glass-strong)', backdropFilter: 'blur(30px) saturate(200%)', borderBottom: '1px solid var(--glass-border)', zIndex: 20 }}>
          <button onClick={onBack} style={{ border: 'none', background: 'transparent', color: 'var(--blue)', cursor: 'pointer', padding: '4px', marginLeft: '-8px' }}>{Vectors.Back}</button>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <div style={{ position: 'relative' }}>
              <button onClick={() => setMenuOpen((v) => !v)} style={{ border: 'none', background: 'transparent', color: 'var(--ink)', cursor: 'pointer', padding: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%' }}>{Vectors.ThreeDots}</button>
              {menuOpen && (
      ...
      {isSearching && (
        <div style={{ background: 'var(--glass-strong)', borderBottom: '1px solid var(--glass-border)', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10, zIndex: 19 }}>
          <div style={{ flex: 1, position: 'relative' }}>
            ...
            />
            {chatSearchQuery && (
              <button onClick={() => setChatSearchQuery('')} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'var(--glass)', border: 'none', borderRadius: '50%', width: 20, height: 20, fontSize: 10, fontWeight: 'bold', color: 'var(--dim)', cursor: 'pointer' }}>✕</button>
            )}
          </div>
          <button onClick={() => { setIsSearching(false); setChatSearchQuery(''); }} style={{ background: 'none', border: 'none', color: 'var(--blue)', fontWeight: 600, cursor: 'pointer', fontSize: 14 }}>Cancel</button>
        </div>
      )}

      <div style={{ position: 'absolute', top: 72, left: 0, right: 0, height: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 5, transform: `translateY(${Math.min(pullDistance - 60, 0)}px)`, opacity: pullDistance > 10 ? 1 : 0, transition: isRefreshing ? 'transform 0.3s cubic-bezier(0.2, 0.8, 0.2, 1)' : 'none', color: 'var(--blue)' }}>
      ...
      <div
        ...
        style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', overscrollBehavior: 'contain', WebkitOverflowScrolling: 'touch', padding: '20px 16px', display: 'flex', flexDirection: 'column-reverse', zIndex: 10, minHeight: 0, backgroundColor: 'var(--bg)', backgroundImage: CHAT_BACKGROUND_IMAGE, backgroundRepeat: 'repeat', transform: `translateY(${pullDistance}px)`, transition: isRefreshing || pullDistance === 0 ? 'transform 0.3s cubic-bezier(0.2, 0.8, 0.2, 1)' : 'none' }}
      >
        ...
        {!messagesLoading && filteredMessages.map((message, index) => {
          ...
          const bubbleBackground = isConfession ? CONFESSION_GRADIENT : (isStickerOrGif ? 'transparent' : (isOwn ? BUBBLE_OWN : BUBBLE_THEM));
          const bubbleColor = isConfession ? '#fff' : (isOwn ? '#fff' : 'var(--ink)');
            ...
            <React.Fragment key={message.id}>
              <div {...longPressHook} onClick={() => { if (selectedMessages.length > 0) toggleSelection(message.id); }}>
                <SwipeableMessage onSwipe={() => { if (selectedMessages.length === 0) startReply(message); }} disabled={isSearching || selectedMessages.length > 0}>
                  <div
                    ...
                    style={{ display: 'flex', flexDirection: isOwn ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: 8, marginBottom: 16, borderRadius: 16, padding: '4px 8px', background: isSelected ? 'rgba(10, 132, 255, 0.15)' : 'transparent', animation: 'slideUpFade 0.3s cubic-bezier(0.2, 0.8, 0.2, 1) both', transition: 'background 0.2s' }}
                  >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: isOwn ? 'flex-end' : 'flex-start', maxWidth: '75%' }}>
                      {!isOwn && (
                        <button onClick={() => { if (!isAnonMsg && selectedMessages.length === 0) setProfileCardUserId(message.user_id); }} disabled={isAnonMsg || selectedMessages.length > 0} style={{ fontSize: 13, fontWeight: 700, marginBottom: 4, marginLeft: 6, color: isAdminMsg ? '#FF8C00' : (isAnonMsg ? 'var(--dim)' : 'var(--blue)'), display: 'flex', alignItems: 'center', gap: 4, border: 'none', background: 'transparent', padding: 0, cursor: isAnonMsg ? 'default' : 'pointer' }}>
                          {isAnonMsg ? 'Anonymous' : (isAdminMsg ? ADMIN_DISPLAY_NAME : message.sender_name)}
                          {isAdminMsg && !isAnonMsg && Vectors.AdminShield}
                        </button>
                      )}

                      <div style={{ maxWidth: '100%', padding: isInstagram ? '4px' : ((message.media_url && !isStickerOrGif) ? '4px' : (isStickerOrGif ? 0 : '10px 16px')), borderRadius: isStickerOrGif ? 0 : 20, borderBottomRightRadius: isStickerOrGif ? 0 : (isOwn ? 4 : 20), borderBottomLeftRadius: isStickerOrGif ? 0 : (isOwn ? 20 : 4), background: bubbleBackground, color: bubbleColor, boxShadow: isStickerOrGif ? 'none' : '0 2px 10px rgba(0,0,0,0.05)' }}>
                        {isConfession && (
                          <div style={{ fontSize: 11, fontWeight: 700, opacity: 0.9, padding: '2px 4px 4px', display: 'flex', alignItems: 'center', gap: 4 }}>{Vectors.GhostSolid && '🤍'} CONFESSION</div>
                        )}

                        {message.reply_to_id && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 2, padding: '6px 10px', marginBottom: 8, marginTop: (message.media_url || isInstagram) ? 4 : 0, borderRadius: 10, background: isOwn || isConfession ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.06)', borderLeft: `3px solid ${isOwn || isConfession ? '#fff' : 'var(--blue)'}` }}>
                            <span style={{ fontSize: 12, fontWeight: 700, color: isOwn || isConfession ? '#fff' : 'var(--blue)' }}>
                              {repliedMessage ? (repliedMessage.is_anon ? 'Anonymous' : repliedMessage.sender_name) : 'Original'}
                            </span>
                            <span className="no-copy-text" style={{ fontSize: 13, color: isOwn || isConfession ? 'rgba(255,255,255,0.85)' : 'var(--dim)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      ...
                      <span style={{ fontSize: 11, color: 'var(--dim)', marginTop: 4, marginInline: 4, fontWeight: 500 }}>{formatTime(message.created_at)}</span>
                    </div>
                  </div>
                </SwipeableMessage>
              </div>
              ...
              {showDayDivider && !isSearching && (
                <div style={{ textAlign: 'center', margin: '24px 0 16px', position: 'sticky', top: 10, zIndex: 5 }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--dim)', background: 'var(--glass-strong)', padding: '6px 14px', borderRadius: 14, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>{formatDayLabel(message.created_at)}</span>
                </div>
              )}
            </React.Fragment>
          );
      ...
      {confessions.length > 0 && (
        <button onClick={handlePinClick} title="Jump to a confession" style={{ position: 'absolute', right: 16, bottom: hasUnreadMention ? 130 : 80, width: 40, height: 40, borderRadius: '50%', background: 'var(--glass-strong)', backdropFilter: 'blur(20px)', border: '1px solid var(--glass-border)', color: 'var(--blue)', zIndex: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 4px 12px rgba(0,0,0,0.12)' }}>
          {Vectors.Pin}
        </button>
      )}

      {hasUnreadMention && (
        <button onClick={handleJumpToMention} style={{ position: 'absolute', right: 16, bottom: 80, width: 40, height: 40, borderRadius: '50%', background: 'var(--blue)', color: '#fff', border: 'none', boxShadow: '0 4px 12px rgba(10,132,255,0.3)', zIndex: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 18, cursor: 'pointer', animation: 'pop-in 0.3s cubic-bezier(0.2, 0.8, 0.2, 1)' }}>
          @
        </button>
      )}
