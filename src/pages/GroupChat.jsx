/**
 * ============================================================================
 * GROUP CHAT MASTER VIEW (GLASS UI)
 * ============================================================================
 * CHANGES IN THIS PASS:
 * - Restyled entirely to the new dark-glass aesthetic using token variables.
 * - Deleted local duplicate subcomponents in favor of shared ones (LiquidAvatar, 
 *   MessageSkeleton, AudioBubble, VideoBubble, InstagramCard, AttachmentSheet, 
 *   SwipeableMessage, SendButton).
 * - Extended ConfessionModal to support attaching a gallery photo which uploads 
 *   to the media_url/photo path.
 * - Rendered confession messages via <ConfessionBubble size="inline">.
 * - Removed floating "pin" button and replaced it with a "Previous Confession"
 *   chip under the main header.
 * - Added <ReactionBar> to every regular message bubble and properly mapped 
 *   the targetId for confession bubbles to target the mirrored confessions row.
 * - Kept 100% of existing realtime subscription, read-receipt, swipe-to-reply, 
...
const Vectors = {
  ...
  CheckCircle: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="var(--ember)" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="12" r="10" stroke="none" /><polyline points="8 12 11 15 16 9" />
    </svg>
  ),
  Instagram: (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function ConfessionModal({ open, onClose, onSubmit }) {
  ...
  if (!open) return null;
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 50, display: 'flex', alignItems: 'flex-end', backdropFilter: 'blur(10px)' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', background: 'var(--ink-2)', borderRadius: '28px 28px 0 0', padding: '24px 20px', borderTop: '1px solid var(--glass-border)', boxShadow: '0 -10px 40px rgba(0,0,0,0.5)' }}>
        <h3 style={{ margin: '0 0 16px', color: 'var(--paper)', fontSize: 20 }}>New Confession</h3>
        <textarea
          value={text} onChange={(e) => setText(e.target.value)} rows={4} placeholder="Type your confession…"
          style={{ width: '100%', border: '1px solid var(--glass-border)', borderRadius: 16, padding: 14, fontSize: 15, resize: 'none', boxSizing: 'border-box', background: 'var(--ink)', color: 'var(--paper)', outline: 'none' }}
        />
        {photo && (
          <div style={{ position: 'relative', marginTop: 12, width: 80, height: 80 }}>
            ...
            <button onClick={() => setPhoto(null)} style={{ position: 'absolute', top: -6, right: -6, background: 'var(--glass-border)', color: 'var(--paper)', border: 'none', borderRadius: '50%', width: 24, height: 24, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{Vectors.Close}</button>
          </div>
        )}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '16px 0' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, color: 'var(--paper)' }}>
            <input type="checkbox" checked={anon} onChange={(e) => setAnon(e.target.checked)} /> Post anonymously
          </label>
          <button onClick={() => photoInputRef.current?.click()} style={{ background: 'var(--glass-border)', border: 'none', color: 'var(--paper)', padding: '8px 12px', borderRadius: 12, cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600 }}>
            {Vectors.Photo}
            Attach Photo
          </button>
        ...
        <button
          onClick={() => { if (text.trim() || photo) { onSubmit(text.trim(), anon, photo); setText(''); setPhoto(null); } }}
          style={{ width: '100%', padding: 16, borderRadius: 20, border: 'none', background: 'var(--ember)', color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 16 }}
        >
          Post Confession
        </button>
      </div>
...
function InstagramModal({ open, onClose, onSubmit, loading }) {
  ...
  if (!open) return null;
  return (
    <div onClick={loading ? undefined : onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 50, display: 'flex', alignItems: 'flex-end', backdropFilter: 'blur(10px)' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', background: 'var(--ink-2)', borderRadius: '28px 28px 0 0', padding: '24px 20px', borderTop: '1px solid var(--glass-border)', boxShadow: '0 -10px 40px rgba(0,0,0,0.5)' }}>
        <h3 style={{ margin: '0 0 6px', color: 'var(--paper)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ color: 'var(--ember)' }}>{Vectors.Instagram}</div> Share Instagram Profile
        </h3>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            style={{ width: '100%', border: '1px solid var(--glass-border)', borderRadius: 16, padding: '14px 14px 14px 32px', fontSize: 15, boxSizing: 'border-box', color: 'var(--paper)', background: 'var(--ink)', outline: 'none' }}
          />
        </div>
        <button
          onClick={() => username.trim() && onSubmit(username.trim())}
          disabled={loading || !username.trim()}
          style={{ width: '100%', marginTop: 16, padding: 16, borderRadius: 20, border: 'none', background: loading ? 'var(--glass-border)' : 'var(--ember)', color: '#fff', fontWeight: 700, cursor: loading ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 16 }}
        >
...
function usePullToRefresh(onRefresh, scrollRef) {
  ...
  const handleTouchStart = (e) => {
    if (scrollRef.current) startY.current = e.touches[0].clientY;
  };
...
export default function GroupChat({ groupSlug, onBack, onGroupResolved }) {
  ...
  // Attachment sheet, confessions, Instagram
      ...
      try {
        ...
        if (isMounted) {
          if (!data) {
            ...
            if (onGroupResolved) onGroupResolved(null);
          } else {
            setGroup(data);
            setGroupStatus('ready');
            if (onGroupResolved) onGroupResolved(data);
          }
        }
      } catch (err) {
  ...
  const fetchMessagesAndReceipts = useCallback(async () => {
    ...
    const { data: receiptData } = await supabase
      .from('group_read_receipts').select('last_read_at')
      .eq('group_id', group.id).eq('user_id', ownUserId).maybeSingle();

    const lastReadAt = receiptData?.last_read_at || '1970-01-01T00:00:00.000Z';

    const { data, error } = await supabase
      .from('group_messages').select('*, profiles(avatar_url)')
      .eq('group_id', group.id).order('created_at', { ascending: false }).limit(MESSAGE_LIMIT);

    if (!error && isMounted) {
      let fetchedMessages = data || [];

      // Map confession IDs to target the mirrored confessions row for <ReactionBar>
      ...
      if (confessionMsgIds.length > 0) {
        ...
        if (confs) {
          fetchedMessages = fetchedMessages.map(m => {
            if (m.is_confession) {
              const match = confs.find(c => c.source_message_id === m.id);
              return { ...m, confession_id: match?.id };
            }
            return m;
          });
        }
      ...
      if (ownUserId) {
        ...
        if (unreadMention) {
          ...
          supabase.from('group_read_receipts').upsert({
            group_id: group.id, user_id: ownUserId, last_read_at: new Date().toISOString()
          }).then();
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`group_messages:${group.id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'group_messages', filter: `group_id=eq.${group.id}` }, async (payload) => {
        ...
        // Delay fetching confession ID slightly to let the DB trigger complete
        if (newMsg.is_confession) {
          setTimeout(async () => {
            ...
            if (cData) {
              setMessages(prev => prev.map(m => m.id === newMsg.id ? { ...m, confession_id: cData.id } : m));
            }
          }, 800);
        }
  ...
  const renderMessageTextWithMentions = (messageText, isOwn) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('@') && part.length > 1) {
        ...
        return (
          <button key={i} onClick={() => handleMentionClick(username)} style={{ color: isOwn ? 'var(--paper)' : 'var(--ember)', textDecoration: 'underline', background: 'none', border: 'none', padding: 0, fontWeight: 700, cursor: 'pointer', fontSize: 'inherit' }}>
            {part}
          </button>
        );
      }
  ...
  const startReply = useCallback((message) => {
    ...
    setReplyingTo({
      id: message.id, sender_name: replyName, text: message.text,
  ...
  async function handleSend(e) {
    ...
    const { error } = await supabase.from('group_messages').insert({
      ...
      text: trimmed, reply_to_id: replyingTo?.id ?? null, mentioned_user_ids: mentionedIds, is_anon: isAnonMode,
    });
  ...
  async function sendPendingAttachment() {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('group_messages').insert({
        group_id: group.id, user_id: session.user.id, sender_name: currentSenderName(),
        text: caption.trim() || null, media_url: publicUrl, media_type: type,
  ...
  async function handleMediaPicked(url, mediaType) {
    ...
    const { error } = await supabase.from('group_messages').insert({
      group_id: group.id, user_id: session.user.id, sender_name: currentSenderName(),
      media_url: url, media_type: mediaType, reply_to_id: replyingTo?.id ?? null, is_anon: isAnonMode,
    });
  ...
  async function handleConfessionSubmit(confessionText, anon, photoFile) {
    ...
    if (photoFile) {
      ...
      try {
        ...
        console.error('Failed to upload confession photo', err);
    ...
    const { error } = await supabase.from('group_messages').insert({
      group_id: group.id, user_id: session.user.id, sender_name: senderName,
      text: confessionText, is_anon: anon, is_confession: true,
  ...
  const confessions = messages.filter((m) => m.is_confession);

  function handlePinClick() {
    if (confessions.length === 0) return;
    const nextIndex = confessionNavIndex + 1 >= confessions.length ? 0 : confessionNavIndex + 1;
    setConfessionNavIndex(nextIndex);
    const target = confessions[nextIndex];
  ...
  const filteredMessages = messages.filter((m) => {
    ...
    if (q.startsWith('@') && q.length > 1) {
      ...
      return m.sender_name?.toLowerCase() === targetName || m.instagram_username?.toLowerCase() === targetName;
    }
  ...
  if (groupStatus === 'loading') {
    return (
      <div className="no-copy-text" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--ink)' }}>
  ...
  if (groupStatus === 'error') {
    return (
      <div className="no-copy-text" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--ink)', flexDirection: 'column', gap: 16 }}>
        <p style={{ color: 'var(--dim)' }}>Failed to load group.</p>
        <button onClick={onBack} style={{ background: 'var(--ember)', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 12, cursor: 'pointer' }}>Go Back</button>
      </div>
    );
  }

  return (
    <div className="no-copy-text" style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative', height: '100%', overflow: 'hidden', zIndex: 1, userSelect: 'none', WebkitUserSelect: 'none', background: 'var(--ink)' }}>
        ...
        <header style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 14, padding: '12px 20px', background: 'var(--glass-strong)', backdropFilter: 'blur(30px) saturate(200%)', borderBottom: '1px solid var(--glass-border)', zIndex: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <button
              ...
              style={{ border: 'none', background: isAnonMode ? 'var(--glass-white)' : 'transparent', color: isAnonMode ? 'var(--ember)' : 'var(--dim)', cursor: 'pointer', padding: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', transition: 'all 0.2s' }}
            ...
            <div style={{ position: 'relative' }}>
              ...
              {menuOpen && (
                <div className="glass-panel" style={{ position: 'absolute', right: 0, top: '100%', marginTop: 4, zIndex: 30, minWidth: 160, padding: 6 }}>
                  <button onClick={() => { setIsSearching(true); setMenuOpen(false); }} className="chat-row" style={{ width: '100%', padding: '10px 14px', border: 'none', background: 'transparent', color: 'var(--paper)', textAlign: 'left', borderRadius: 8, cursor: 'pointer', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
                    {Vectors.SearchSmall} Search Chat
                  </button>
                  <button onClick={() => { navigator.clipboard.writeText(window.location.href); setMenuOpen(false); showToast('Link copied to clipboard!', 'info'); }} className="chat-row" style={{ width: '100%', padding: '10px 14px', border: 'none', background: 'transparent', color: 'var(--paper)', textAlign: 'left', borderRadius: 8, cursor: 'pointer', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {isSearching && (
        <div style={{ background: 'var(--glass-strong)', borderBottom: '1px solid var(--glass-border)', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10, zIndex: 19 }}>
          <div style={{ flex: 1, position: 'relative' }}>
            ...
            <input
              autoFocus type="text" value={chatSearchQuery} onChange={(e) => setChatSearchQuery(e.target.value)}
              placeholder="Search or type @username..."
              style={{ width: '100%', padding: '8px 12px 8px 36px', borderRadius: 16, border: 'none', background: 'var(--ink-2)', color: 'var(--paper)', fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
            />
            {chatSearchQuery && (
              <button onClick={() => setChatSearchQuery('')} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'var(--glass-border)', border: 'none', borderRadius: '50%', width: 20, height: 20, fontSize: 10, fontWeight: 'bold', color: 'var(--paper)', cursor: 'pointer' }}>✕</button>
            )}
          </div>
      ...
      {confessions.length > 0 && !isSearching && (
        <div style={{ padding: '8px 16px', background: 'var(--glass-strong)', borderBottom: '1px solid var(--glass-border)', zIndex: 19, display: 'flex' }}>
          <button
             onClick={handlePinClick}
             style={{
               background: 'var(--glass-white)', border: '1px solid var(--glass-border)',
               color: 'var(--paper)', borderRadius: 20, padding: '8px 16px', fontSize: 13,
      ...
      >
        {messagesLoading && <MessageSkeleton variant="message" count={4} />}

        {!messagesLoading && filteredMessages.length === 0 && (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: 'var(--glass-white)', padding: '8px 16px', borderRadius: 20, fontSize: 14, color: 'var(--dim)', border: '1px solid var(--glass-border)' }}>
        ...
        {!messagesLoading && filteredMessages.map((message, index) => {
          ...
          const bubbleBackground = isStickerOrGif ? 'transparent' : (isOwn ? 'color-mix(in srgb, var(--ember) 18%, var(--glass-white))' : 'var(--glass-white)');
          const bubbleColor = 'var(--paper)';

          return (
            <React.Fragment key={message.id}>
              <div {...longPressHook} onClick={() => { if (selectedMessages.length > 0) toggleSelection(message.id); }}>
                <SwipeableMessage onSwipe={() => { if (selectedMessages.length === 0) startReply(message); }} disabled={isSearching || selectedMessages.length > 0}>
                  ...
                  {isConfession ? (
                     <div
                        ...
                        style={{ background: isSelected ? 'rgba(255,107,53, 0.15)' : 'transparent', borderRadius: 16 }}
                     >
                       <ConfessionBubble
                          confession={{
                              id: message.confession_id || message.id, 
                              text: message.text,
                              photo_url: message.media_url,
                              is_anon: message.is_anon,
                              created_at: message.created_at
                          }}
                          onReply={() => { if (selectedMessages.length === 0) startReply(message); }}
                          userId={ownUserId}
                          size="inline"
                       />
                     </div>
                    ...
                    >
                      {selectedMessages.length > 0 && isAdmin && (
                        <div style={{ margin: '0 8px 16px', color: isSelected ? 'var(--ember)' : 'var(--glass-border)' }}>
                          {isSelected ? Vectors.CheckCircle : <div style={{ width: 20, height: 20, borderRadius: '50%', border: '2px solid currentColor' }} />}
                        </div>
                      )}

                      {!isOwn && (
                        <button onClick={() => { if (!isAnonMsg && selectedMessages.length === 0) setProfileCardUserId(message.user_id); }} disabled={isAnonMsg || selectedMessages.length > 0} style={{ border: 'none', background: 'transparent', padding: 0, cursor: isAnonMsg ? 'default' : 'pointer', marginBottom: 20 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: isOwn ? 'flex-end' : 'flex-start', maxWidth: '85%' }}>
                        {!isOwn && (
                          <button onClick={() => { if (!isAnonMsg && selectedMessages.length === 0) setProfileCardUserId(message.user_id); }} disabled={isAnonMsg || selectedMessages.length > 0} style={{ fontSize: 13, fontWeight: 700, marginBottom: 4, marginLeft: 6, color: isAdminMsg ? '#FFD700' : (isAnonMsg ? 'var(--dim)' : 'var(--paper)'), display: 'flex', alignItems: 'center', gap: 4, border: 'none', background: 'transparent', padding: 0, cursor: isAnonMsg ? 'default' : 'pointer' }}>
                            {isAnonMsg ? 'Anonymous' : (isAdminMsg ? ADMIN_DISPLAY_NAME : message.sender_name)}
                            {isAdminMsg && !isAnonMsg && Vectors.AdminShield}
                          </button>
                        )}

                        <div style={{ maxWidth: '100%', padding: isInstagram ? '4px' : ((message.media_url && !isStickerOrGif) ? '4px' : (isStickerOrGif ? 0 : '10px 16px')), borderRadius: isStickerOrGif ? 0 : 20, borderBottomRightRadius: isStickerOrGif ? 0 : (isOwn ? 4 : 20), borderBottomLeftRadius: isStickerOrGif ? 0 : (isOwn ? 20 : 4), background: bubbleBackground, color: bubbleColor, border: isStickerOrGif ? 'none' : '1px solid var(--glass-border)', boxShadow: isStickerOrGif ? 'none' : '0 6px 18px rgba(0,0,0,0.35)' }}>
                          {message.reply_to_id && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 2, padding: '6px 10px', marginBottom: 8, marginTop: (message.media_url || isInstagram) ? 4 : 0, borderRadius: 10, background: 'var(--glass-white)', borderLeft: `3px solid var(--paper)` }}>
                              <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--paper)' }}>
                                {repliedMessage ? (repliedMessage.is_anon ? 'Anonymous' : repliedMessage.sender_name) : 'Original'}
                              </span>
                              <span className="no-copy-text" style={{ fontSize: 13, color: 'var(--dim)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          ...
                          {isInstagram ? (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: message.text ? 6 : 0 }}>
                              {isStickerOrGif ? (
                                ...
                                <a href={message.media_url} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: 'var(--glass-white)', borderRadius: 16, textDecoration: 'none', border: '1px solid var(--glass-border)' }}>
                                  <div style={{ color: 'var(--paper)' }}>{Vectors.FileText}</div>
                                  <span style={{ color: 'var(--paper)', fontSize: 14, fontWeight: 600 }}>Document</span>
                                </a>
                              )}
                              {message.text && (
                                <span className="no-copy-text" style={{ fontSize: 14, whiteSpace: 'pre-wrap', wordBreak: 'break-word', padding: '0 4px' }}>
                          ...
                          {!isStickerOrGif && (
                             <div style={{ marginTop: 8, padding: '0 4px' }}>
                                 <ReactionBar targetType="group_message" targetId={message.id} userId={ownUserId} />
                             </div>
                          )}
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                          ...
                          <span style={{ fontSize: 11, color: 'var(--dim)', marginInline: 4, fontWeight: 500 }}>{formatTime(message.created_at)}</span>
                        </div>
                      </div>
                    </div>
                  )}
              ...
              {showDayDivider && !isSearching && (
                <div style={{ textAlign: 'center', margin: '24px 0 16px', position: 'sticky', top: 10, zIndex: 5 }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--dim)', background: 'var(--glass-strong)', padding: '6px 14px', borderRadius: 14, border: '1px solid var(--glass-border)' }}>{formatDayLabel(message.created_at)}</span>
                </div>
              )}
            </React.Fragment>
          );
      ...
      {hasUnreadMention && (
        <button onClick={handleJumpToMention} style={{ position: 'absolute', right: 16, bottom: 80, width: 40, height: 40, borderRadius: '50%', background: 'var(--ember)', color: '#fff', border: 'none', boxShadow: '0 6px 18px rgba(0,0,0,0.35)', zIndex: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 18, cursor: 'pointer', animation: 'pop-in 0.3s cubic-bezier(0.2, 0.8, 0.2, 1)' }}>
          @
        </button>
      )}
      ...
      <div className="safe-bottom" style={{ flexShrink: 0, zIndex: 20, position: 'sticky', bottom: 0 }}>
        {!session ? (
          <div style={{ padding: '16px', background: 'var(--glass-strong)', backdropFilter: 'blur(30px) saturate(200%)', borderTop: '1px solid var(--glass-border)' }}>
            <button onClick={() => setAuthOpen(true)} style={{ width: '100%', padding: '14px 0', borderRadius: 20, border: 'none', background: 'var(--ember)', color: '#fff', fontWeight: 700, fontSize: 15, cursor: 'pointer', boxShadow: '0 6px 18px rgba(0,0,0,0.35)' }}>
          ...
          <>
              ...
              <div style={{ position: 'absolute', bottom: '100%', left: 0, right: 0, background: 'var(--glass-strong)', backdropFilter: 'blur(20px)', borderTop: '1px solid var(--glass-border)', padding: '12px 16px', zIndex: 21 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                  {pendingFile.type === 'image' ? (
                    ...
                    <div style={{ width: 56, height: 56, borderRadius: 12, background: 'var(--glass-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--paper)' }}>{Vectors.Smiley}</div>
                  ) : (
                    <div style={{ width: 56, height: 56, borderRadius: 12, background: 'var(--glass-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--paper)' }}>{Vectors.FileText}</div>
                  )}
                  ...
                  <button onClick={cancelPendingAttachment} disabled={uploading} style={{ border: 'none', background: 'var(--glass-border)', width: 28, height: 28, borderRadius: '50%', color: 'var(--paper)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{Vectors.Close}</button>
                </div>
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  <input type="text" value={caption} onChange={(e) => setCaption(e.target.value)} placeholder="Add a caption…" disabled={uploading} style={{ flex: 1, border: '1px solid var(--glass-border)', outline: 'none', background: 'var(--ink-2)', borderRadius: 20, padding: '10px 16px', fontSize: 14, color: 'var(--paper)' }} />
                  <button type="button" onClick={sendPendingAttachment} disabled={uploading} style={{ width: 44, height: 44, borderRadius: '50%', border: 'none', background: uploading ? 'var(--glass-border)' : 'var(--ember)', color: '#fff', cursor: uploading ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <div style={{ position: 'absolute', bottom: pendingFile ? undefined : '100%', top: pendingFile ? '100%' : undefined, left: 0, right: 0, background: 'var(--glass-strong)', backdropFilter: 'blur(20px)', borderTop: '1px solid var(--glass-border)', display: 'flex', alignItems: 'center', padding: '10px 16px', gap: 12, transition: 'all 0.35s cubic-bezier(0.175, 0.885, 0.32, 1.1)', transform: replyingTo && !pendingFile ? 'translateY(0)' : 'translateY(100%)', opacity: replyingTo && !pendingFile ? 1 : 0, visibility: replyingTo && !pendingFile ? 'visible' : 'hidden', zIndex: 19 }}>
              ...
              <button onClick={() => setReplyingTo(null)} style={{ border: 'none', background: 'var(--glass-border)', width: 28, height: 28, borderRadius: '50%', color: 'var(--paper)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{Vectors.Close}</button>
            </div>
            ...
            <form onSubmit={handleSend} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: 'var(--glass-strong)', backdropFilter: 'blur(30px) saturate(200%)', borderTop: replyingTo ? 'none' : '1px solid var(--glass-border)', position: 'relative', zIndex: 20 }}>
              ...
              <button type="button" onClick={() => setAttachSheetOpen(true)} disabled={uploading || cooldownPercent > 0 || selectedMessages.length > 0} style={{ width: 36, height: 36, borderRadius: '50%', border: 'none', background: 'transparent', color: 'var(--dim)', cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
