...
//   - mode="message", plain      -> "Message"  (ember/blue)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
const NEUTRAL_BASE = 'var(--ink-2)';
const NEUTRAL_ACCENT = 'rgba(61,139,255,0.5)';
...
function structureSwatchStyle(bg) {
  switch (bg.type) {
    ...
    case 'linear':
      return { background: `linear-gradient(135deg, ${NEUTRAL_BASE}, var(--ink))` };
    case 'radial':
      return { background: `radial-gradient(circle at 35% 30%, var(--ember), ${NEUTRAL_BASE})` };
    case 'dots':
    case 'halftone':
    ...
    case 'confetti':
      return {
        background: NEUTRAL_BASE,
        backgroundImage: 'radial-gradient(var(--ember) 1.4px, transparent 1.4px), radial-gradient(#8B5CF6 1.4px, transparent 1.4px), radial-gradient(#2DD4A7 1.4px, transparent 1.4px)',
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div
          ...
          style={{
            ...
            boxShadow: '0 0 0 2px rgba(61,139,255,0.25)',
            pointerEvents: 'none',
            zIndex: 2,
          }}
        />
...
function shapeSwatchCardStyle(shape) {
  ...
  if (shape.fill === 'ink-2') return { ...base, background: 'var(--ink-2)' };
  if (shape.fill === 'paper') return { ...base, background: 'var(--paper)' };
  if (shape.fill === 'glass') return { ...base, background: 'rgba(255,255,255,0.09)' };
  if (shape.fill === 'gradient-header') return { ...base, background: 'linear-gradient(160deg, rgba(61,139,255,0.4), var(--ink-2))' };
  if (shape.fill === 'radial-glow') return { ...base, background: 'radial-gradient(circle at 35% 30%, rgba(61,139,255,0.55), var(--ink-2))' };
  if (shape.fill === 'split') return { ...base, background: 'linear-gradient(to bottom, rgba(61,139,255,0.5) 36%, var(--ink-2) 36%)' };
...
function shapeSwatchBorderStyle(border) {
  ...
  if (border === 'glow') return { border: '1.5px solid var(--ember)', boxShadow: '0 0 8px rgba(61,139,255,0.55)' };
  ...
  if (border === 'ink-thick') return { border: '3px solid var(--ink)' };
  if (border === 'paper-thick') return { border: '3px solid var(--paper)' };
    ...
    >
      <span
        style={{
          ...
          color: shape.darkText ? 'var(--ink)' : 'var(--paper)',
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div
          ...
          style={{
            ...
            boxShadow: '0 0 8px rgba(61,139,255,0.4)',
            pointerEvents: 'none',
            zIndex: 2,
          }}
        />
            ...
            return (
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  padding: 0,
                  border: selected ? '3px solid var(--paper)' : '2px solid rgba(90,150,255,0.20)',
