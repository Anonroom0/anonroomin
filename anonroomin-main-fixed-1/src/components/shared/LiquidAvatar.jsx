 ...
 *   2. identity.is_admin (kind="user" only) + identity.avatar_url — admin's
 *      real profile photo, ringed in gold with a small shield badge so it
 *      still reads as "admin" at a glance
 *   3. identity.is_admin with no avatar_url — solid gold fill, "ADM" label
 *   4. identity.avatar_url — the actual photo/cover image
 *   5. fallback — colored gradient with initials ("#" for kind="group")
 *
...
const FALLBACK_COLOR = 'var(--ember)';
...
export default function LiquidAvatar({
  ...
  } else if (isAdmin && avatarUrl) {
    // Admin's actual profile photo — kept identifiable as admin via a gold
    // ring (below) plus a small shield badge, instead of hiding the photo
    // behind the flat "ADM" fallback.
    content = (
      <div style={{ ...circleStyle, boxShadow: 'inset 0 0 0 2px var(--admin-1)' }}>
        <img src={avatarUrl} alt={name || 'Admin'} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
      </div>
    );
  } else if (isAdmin) {
    content = (
  ...
  const showAdminBadge = isAdmin && avatarUrl && size >= 28;
  ...
  return (
    ...
    >
      ...
      {showAdminBadge && (
        ...
        >
          <svg width={badgeSize * 0.6} height={badgeSize * 0.6} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
